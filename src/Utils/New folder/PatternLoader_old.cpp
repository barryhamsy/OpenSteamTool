...
// Set to true the first time any Load() call resolves patterns from a
// remote mirror (GitHub/jsDelivr/custom). Stays false if every Load()
// fell back to the local cache. Snapshotted at init; never reset.
static bool g_remoteEverSucceeded = false;
