...
// True once any Load() this session fetched its TOML from the remote
// source rather than the local cache. Snapshotted indirectly via
// RemoteToml::Result::fromCache — see Load() below. Read by
// PatternLoader::WasRemoteReachable(), which hooks consult as a
// coarse "are we online enough for Steam's backend to respond?" probe.
static bool g_anyRemoteSuccess = false;
...
bool Load(HMODULE module, const std::string& dllPath, const std::string& component)
{
    ...
    if (r.ok) {
        ...
        if (!map.empty()) {
            ...
            // Latch remote-reachability for the session if the body came
            // straight from the network rather than the on-disk cache.
            if (!r.fromCache) g_anyRemoteSuccess = true;
