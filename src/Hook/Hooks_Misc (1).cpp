...
namespace {
    ...
    // True  = at least one Load() this session fetched its TOML from the
    //         network → treat session as online → GetOrAddAppData is a
    //         passthrough (PICS will resolve newly added depots naturally).
    // False = every Load() fell back to the local cache → treat session
    //         as offline → GetOrAddAppData applies skip_flag so license
    //         updates complete without waiting for PICS responses that
    //         will never arrive.
    ...
    //
    // GATED ON OFFLINE STATE (g_IsOffline).
    //
    // The injected package keeps Lua-provided ids in PackageInfo::AppIdVec.
    // Some of those ids can actually be depot ids; we cannot trust the Lua
    // config to classify app ids and depot ids for us. In OFFLINE mode,
    // depot ids usually have only placeholder appinfo, which blocks
    // CClientAppManager_ProcessPendingLicenseUpdates because it waits for
    // every AppIdVec entry to have resolved appinfo unless marked unknown.
    // Setting skip_flag for unresolved entries gets the license update
    // through.
    //
    // In ONLINE mode we deliberately do nothing — PICS will resolve
    // placeholder appinfo for newly added depots, and applying skip_flag
    // would prematurely condemn brand-new AppIDs to "known-unknown" before
    // PICS had a chance, silently breaking new-game unlocks.
...
namespace Hooks_Misc {
    void Install() {
        ...
        // Hooks_Misc::Install() runs (see dllmain init order).
